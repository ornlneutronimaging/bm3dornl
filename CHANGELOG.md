...
- Multiscale: `bm3d_ring_artifact_removal(..., multiscale=True)` with `sigma_random=0.0` (automatic noise estimation) no longer returns flat-background sinograms unchanged. The multiscale path's internal estimator tiles the image into patches and takes the minimum patch sigma as the air-region noise floor; on simulated sinograms the air is numerically flat, so that minimum was float quantization residue (~1e-10), the variance lock's threshold collapsed with it, and the whole correction was silently suppressed — a second benchmark row measured on an untouched input. Patch sigmas at or below 1e-6 of the input's value range are now excluded from the minimum as quantization residue, not noise floors (measured over the full 540-slice CG-1D volume: flat-air patches sit near 3e-10 of the range; the quietest patch of any measured slice sits at 3.7e-4). Outputs on measured data are byte-identical to the previous release. On flat-air inputs — the degenerate family this targets — automatic-sigma outputs change by design, and explicit-sigma outputs can also change marginally where the pre-existing per-scale estimate override (which replaces the caller's sigma whenever the per-scale estimate exceeds 0.001) now sees a genuine estimate instead of residue. If every patch is silent there is nothing to denoise and the input is returned as before.
