...
def generate_synthetic_sinogram(size=(256, 256), snr_db=10):
    ...
    x = np.linspace(-1, 1, size[1], dtype=np.float32)
    y = np.linspace(-1, 1, size[0], dtype=np.float32)
    X, Y = np.meshgrid(x, y)
    ...
    np.random.seed(42)
    streak_profile = (np.random.rand(size[1]).astype(np.float32) - 0.5) * 0.2
    streak_profile = gaussian_filter(streak_profile, 1.0).astype(np.float32)
    ...
    return clean, noisy_streaky, np.float32(noise_sigma)
