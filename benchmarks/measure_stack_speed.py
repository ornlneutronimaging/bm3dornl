...
def test_stack_speed():
    ...
    print("Processing stack (Mode=streak, Patch=8x8, max_matches=32)...")
    start = time.time()
    denoised_stack_m32 = bm3d_ring_artifact_removal(
        stack_noisy, mode="streak", sigma_random=sigma, patch_size=8, max_matches=32
    )
    end = time.time()
    ...
    print("Processing stack (Mode=streak, Patch=8x8, step_size=3)...")
    start = time.time()
    denoised_stack_s3 = bm3d_ring_artifact_removal(
        stack_noisy, mode="streak", sigma_random=sigma, patch_size=8, step_size=3
    )
    end = time.time()
